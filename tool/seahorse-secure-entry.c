...
static void
seahorse_secure_entry_editable_init (GtkEditableClass *iface)
{
    iface->do_insert_text = seahorse_secure_entry_insert_text;
    iface->do_delete_text = seahorse_secure_entry_delete_text;
    iface->insert_text = seahorse_secure_entry_real_insert_text;
    iface->delete_text = seahorse_secure_entry_real_delete_text;
    iface->set_selection_bounds = seahorse_secure_entry_set_selection_bounds;
    iface->get_selection_bounds = seahorse_secure_entry_get_selection_bounds;
    iface->set_position = seahorse_secure_entry_real_set_position;
    iface->get_position = seahorse_secure_entry_get_position;
}

static void
seahorse_secure_entry_cell_editable_init (GtkCellEditableIface * iface)
{
    iface->start_editing = seahorse_secure_entry_start_editing;
}

static void
seahorse_secure_entry_set_property (GObject *object, guint prop_id,
                                    const GValue *value, GParamSpec *pspec)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY(object);

    switch (prop_id) {
    case PROP_MAX_LENGTH:
        seahorse_secure_entry_set_max_length(entry, g_value_get_int(value));
        break;

    case PROP_HAS_FRAME:
        seahorse_secure_entry_set_has_frame(entry, g_value_get_boolean(value));
        break;

    case PROP_INVISIBLE_CHAR:
        seahorse_secure_entry_set_invisible_char(entry, g_value_get_uint(value));
        break;

    case PROP_ACTIVATES_DEFAULT:
        seahorse_secure_entry_set_activates_default(entry, g_value_get_boolean(value));
        break;

    case PROP_WIDTH_CHARS:
        seahorse_secure_entry_set_width_chars(entry, g_value_get_int(value));
        break;
...
static void
seahorse_secure_entry_get_property (GObject *object, guint prop_id,
                                    GValue *value, GParamSpec *pspec)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY(object);
...
static void
seahorse_secure_entry_finalize (GObject *object)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (object);
...
static void
seahorse_secure_entry_realize (GtkWidget *widget)
{
    ...
    entry = SEAHORSE_SECURE_ENTRY (widget);
    ...
    seahorse_secure_entry_adjust_scroll (entry);
}

static void
seahorse_secure_entry_unrealize (GtkWidget *widget)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);

    seahorse_secure_entry_reset_layout (entry);
...
static void
get_borders (SeahorseSecureEntry *entry, gint *xborder, gint *yborder)
{
...
static void
seahorse_secure_entry_size_request (GtkWidget *widget, GtkRequisition *requisition)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);
...
static void
get_text_area_size(SeahorseSecureEntry *entry, gint *x, gint *y, gint *width, gint *height)
{
    gint xborder, yborder;
...
static void
get_widget_window_size (SeahorseSecureEntry *entry, gint *x, gint *y, gint *width, gint *height)
{
    GtkRequisition requisition;
    GtkWidget *widget = GTK_WIDGET (entry);
...
static void
seahorse_secure_entry_size_allocate(GtkWidget *widget, GtkAllocation *allocation)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY(widget);
...
static void
seahorse_secure_entry_draw_frame (GtkWidget *widget)
{
    gint x = 0, y = 0;
    gint width, height;
    gboolean interior_focus;
...
static gint
seahorse_secure_entry_expose(GtkWidget *widget, GdkEventExpose *event)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY(widget);

    if (widget->window == event->window)
        seahorse_secure_entry_draw_frame(widget);
        ...
        if ((entry->invisible_char != 0) && GTK_WIDGET_HAS_FOCUS (widget) &&
            ...
            seahorse_secure_entry_draw_cursor (SEAHORSE_SECURE_ENTRY (widget));

        seahorse_secure_entry_draw_text (SEAHORSE_SECURE_ENTRY (widget));
    }
...
static gint
seahorse_secure_entry_button_press(GtkWidget *widget, GdkEventButton *event)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);
    gint tmp_pos;
    ...
    tmp_pos = seahorse_secure_entry_find_position (entry, event->x + entry->scroll_offset);

    if (event->button == 1) {
        switch (event->type) {
        case GDK_BUTTON_PRESS:
            seahorse_secure_entry_set_positions(entry, tmp_pos, tmp_pos);
            break;
        default:
            break;
        }
...
static gint
seahorse_secure_entry_button_release(GtkWidget *widget, GdkEventButton *event)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY(widget);
...
static gint
seahorse_secure_entry_motion_notify(GtkWidget *widget, GdkEventMotion *event)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY(widget);
    gint tmp_pos;
...

    {
        ...
        if (event->y < 0)
            ...
            tmp_pos = seahorse_secure_entry_find_position (entry,
                                                event->x + entry->scroll_offset);

        seahorse_secure_entry_set_positions (entry, tmp_pos, -1);
    }
...
static void
seahorse_secure_entry_obscure_mouse_cursor (SeahorseSecureEntry * entry)
{
    if (entry->mouse_cursor_obscured)
        return;
...
static gint
seahorse_secure_entry_key_press (GtkWidget *widget, GdkEventKey *event)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);

    seahorse_secure_entry_pend_cursor_blink (entry);

    if (gtk_im_context_filter_keypress (entry->im_context, event)) {
        seahorse_secure_entry_obscure_mouse_cursor (entry);
...
static gint
seahorse_secure_entry_key_release (GtkWidget *widget, GdkEventKey *event)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);
...
static gint
seahorse_secure_entry_focus_in (GtkWidget *widget, GdkEventFocus *event)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);
    ...
    g_signal_connect (gdk_keymap_get_for_display (gtk_widget_get_display (widget)), "direction_changed",
                      G_CALLBACK (seahorse_secure_entry_keymap_direction_changed), entry);

    seahorse_secure_entry_check_cursor_blink (entry);
...
seahorse_secure_entry_focus_out (GtkWidget *widget, GdkEventFocus *event)
{
    ...
    g_signal_handlers_disconnect_by_func (gdk_keymap_get_for_display (gtk_widget_get_display (widget)),
                                          seahorse_secure_entry_keymap_direction_changed, entry);

    return FALSE;
}

static void
seahorse_secure_entry_grab_focus (GtkWidget *widget)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);
...
static void
seahorse_secure_entry_direction_changed(GtkWidget *widget, GtkTextDirection previous_dir)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);
...
static void
seahorse_secure_entry_state_changed (GtkWidget *widget, GtkStateType previous_state)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);
...
static void
seahorse_secure_entry_screen_changed (GtkWidget *widget, GdkScreen *old_screen)
{
    seahorse_secure_entry_recompute (SEAHORSE_SECURE_ENTRY (widget));
}

/* GtkEditable method implementations */

static void
seahorse_secure_entry_insert_text (GtkEditable *editable, const gchar *new_text,
                                   gint new_text_length, gint * position)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY(editable);
    gchar *text;
...
static void
seahorse_secure_entry_delete_text (GtkEditable* editable, gint start_pos,
                                   gint end_pos)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (editable);
...
static void
seahorse_secure_entry_set_position_internal (SeahorseSecureEntry *entry,
                                             gint position, gboolean reset_im)
{
    ...
    if (position != entry->current_pos || position != entry->selection_bound) {
        if (reset_im)
            seahorse_secure_entry_reset_im_context (entry);
        seahorse_secure_entry_set_positions (entry, position, position);
    }
}

static void
seahorse_secure_entry_real_set_position (GtkEditable *editable, gint position)
{
    seahorse_secure_entry_set_position_internal (SEAHORSE_SECURE_ENTRY (editable),
                                                 position, TRUE);
}

static gint
seahorse_secure_entry_get_position (GtkEditable *editable)
{
    return SEAHORSE_SECURE_ENTRY (editable)->current_pos;
}

static void
seahorse_secure_entry_set_selection_bounds (GtkEditable *editable,
                                            gint start, gint end)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (editable);
    ...
    seahorse_secure_entry_set_positions (entry, MIN (end, entry->text_length),
                                         MIN (start, entry->text_length));
}

static gboolean
seahorse_secure_entry_get_selection_bounds (GtkEditable *editable,
                                            gint *start, gint *end)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (editable);
...
static void
seahorse_secure_entry_style_set (GtkWidget *widget, GtkStyle *previous_style)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (widget);
 ...
 */
static void
gtk_cell_editable_secure_entry_activated (SeahorseSecureEntry *entry, gpointer data)
{
...
static void
seahorse_secure_entry_start_editing (GtkCellEditable *cell_editable,
                                     GdkEvent *event)
{
    SEAHORSE_SECURE_ENTRY(cell_editable)->is_cell_renderer = TRUE;
...
static void
seahorse_secure_entry_real_insert_text (GtkEditable *editable, const gchar *new_text,
                                        gint new_text_length, gint *position)
{
    gint _index;
    gint n_chars;
...
static void
seahorse_secure_entry_real_delete_text (GtkEditable *editable, gint start_pos,
                                        gint end_pos)
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY (editable);
    ...
    if (start_pos < end_pos) {
        ...
        seahorse_secure_entry_set_positions(entry, current_pos, selection_bound);
 ...
 */
static gint
get_better_cursor_x (SeahorseSecureEntry *entry, gint offset)
{
    GdkKeymap *keymap = gdk_keymap_get_for_display (gtk_widget_get_display (GTK_WIDGET(entry)));
    PangoDirection keymap_direction = gdk_keymap_get_direction (keymap);
    gboolean split_cursor;

    PangoLayout *layout = seahorse_secure_entry_ensure_layout (entry, TRUE);
...
static void
seahorse_secure_entry_move_cursor (SeahorseSecureEntry *entry, GtkMovementStep step,
                                   gint count, gboolean extend_selection)
{
    ...
    if (entry->current_pos != entry->selection_bound && !extend_selection) {
        ...
        switch (step) {
        case GTK_MOVEMENT_LOGICAL_POSITIONS:
            new_pos = seahorse_secure_entry_move_logically (entry, new_pos, count);
            break;
        case GTK_MOVEMENT_VISUAL_POSITIONS:
            new_pos = seahorse_secure_entry_move_visually (entry, new_pos, count);
            break;
        case GTK_MOVEMENT_DISPLAY_LINE_ENDS:
        case GTK_MOVEMENT_PARAGRAPH_ENDS:
        case GTK_MOVEMENT_BUFFER_ENDS:
...
static void
seahorse_secure_entry_insert_at_cursor(SeahorseSecureEntry *entry, const gchar *str)
{
...
static void
seahorse_secure_entry_delete_from_cursor (SeahorseSecureEntry *entry, GtkDeleteType type,
                                          gint count)
{
    GtkEditable *editable = GTK_EDITABLE (entry);
    ...
    case GTK_DELETE_CHARS:
        end_pos = seahorse_secure_entry_move_logically (entry, entry->current_pos, count);
...
#if UNUSED
static void
seahorse_secure_entry_delete_cb (SeahorseSecureEntry *entry)
{
    GtkEditable *editable = GTK_EDITABLE (entry);
    gint start, end;
...
static void
seahorse_secure_entry_toggle_overwrite (SeahorseSecureEntry *entry)
{
...
static void
seahorse_secure_entry_real_activate (SeahorseSecureEntry *entry)
{
    GtkWindow *window;
    GtkWidget *toplevel;
    GtkWidget *widget;
...
static void
seahorse_secure_entry_keymap_direction_changed (GdkKeymap *keymap, SeahorseSecureEntry *entry)
{
    seahorse_secure_entry_recompute (entry);
}
...
static void
seahorse_secure_entry_commit_cb (GtkIMContext *context, const gchar *str,
                                 SeahorseSecureEntry *entry)
{
    seahorse_secure_entry_enter_text (entry, str);
}

static void
seahorse_secure_entry_preedit_changed_cb (GtkIMContext *context, SeahorseSecureEntry *entry)
{
...
static gboolean
seahorse_secure_entry_retrieve_surrounding_cb (GtkIMContext *context, SeahorseSecureEntry *entry)
{
...
static gboolean
seahorse_secure_entry_delete_surrounding_cb (GtkIMContext *slave, gint offset, gint n_chars,
                                             SeahorseSecureEntry *entry)
{
...
static void
seahorse_secure_entry_enter_text (SeahorseSecureEntry *entry, const gchar *str)
{
    GtkEditable *editable = GTK_EDITABLE (entry);
    gint tmp_pos;
    ...
    if (gtk_editable_get_selection_bounds (editable, NULL, NULL))
        ...
        if (entry->overwrite_mode)
            seahorse_secure_entry_delete_from_cursor (entry, GTK_DELETE_CHARS, 1);
    }

    tmp_pos = entry->current_pos;
    gtk_editable_insert_text (editable, str, strlen(str), &tmp_pos);
    seahorse_secure_entry_set_position_internal (entry, tmp_pos, FALSE);
}
 ...
 */
static void
seahorse_secure_entry_set_positions (SeahorseSecureEntry *entry, gint current_pos,
                                     gint selection_bound)
{
...
static void
seahorse_secure_entry_reset_layout (SeahorseSecureEntry *entry)
{
    if (entry->cached_layout) {
        g_object_unref (entry->cached_layout);
...
static void
update_im_cursor_location (SeahorseSecureEntry *entry)
{
    GdkRectangle area;
    gint strong_x;
    gint strong_xoffset;
    ...
    seahorse_secure_entry_get_cursor_locations (entry, &strong_x, NULL);
    get_text_area_size (entry, NULL, NULL, &area_width, &area_height);
...
recompute_idle_func (gpointer data)
{
    ...
    entry = SEAHORSE_SECURE_ENTRY (data);

    entry->recompute_idle = 0;

    if (gtk_widget_has_screen (GTK_WIDGET (entry))) {
        seahorse_secure_entry_adjust_scroll (entry);
        seahorse_secure_entry_queue_draw (entry);
...
static void
seahorse_secure_entry_recompute (SeahorseSecureEntry *entry)
{
    seahorse_secure_entry_reset_layout (entry);
    seahorse_secure_entry_check_cursor_blink (entry);
...
static PangoLayout *
seahorse_secure_entry_create_layout (SeahorseSecureEntry * entry, gboolean include_preedit)
{
    GtkWidget *widget = GTK_WIDGET (entry);
...
static PangoLayout *
seahorse_secure_entry_ensure_layout (SeahorseSecureEntry *entry, gboolean include_preedit)
{
    if (entry->preedit_length > 0 && !include_preedit != !entry->cache_includes_preedit)
        seahorse_secure_entry_reset_layout (entry);

    if (!entry->cached_layout) {
        entry->cached_layout = seahorse_secure_entry_create_layout (entry, include_preedit);
...
static void
get_layout_position (SeahorseSecureEntry *entry, gint *x, gint *y)
{
    PangoLayout *layout;
    ...
    layout = seahorse_secure_entry_ensure_layout (entry, TRUE);
...
static void
seahorse_secure_entry_draw_text(SeahorseSecureEntry *entry)
{
    GtkWidget *widget;
    PangoLayoutLine *line;
...
static void
draw_insertion_cursor (SeahorseSecureEntry *entry, GdkRectangle *cursor_location,
...
static void
seahorse_secure_entry_draw_cursor (SeahorseSecureEntry * entry)
{
    GdkKeymap *keymap = gdk_keymap_get_for_display (gtk_widget_get_display (GTK_WIDGET(entry)));
    PangoDirection keymap_direction = gdk_keymap_get_direction (keymap);
    ...
    if (GTK_WIDGET_DRAWABLE (entry)) {
        ...
        seahorse_secure_entry_get_cursor_locations (entry, &strong_x, &weak_x);
...
static void
seahorse_secure_entry_queue_draw (SeahorseSecureEntry *entry)
{
...
static void
seahorse_secure_entry_reset_im_context (SeahorseSecureEntry *entry)
{
...
static gint
seahorse_secure_entry_find_position (SeahorseSecureEntry *entry, gint x)
{
    PangoLayout *layout;
    PangoLayoutLine *line;
    gint _index;
...
static void
seahorse_secure_entry_get_cursor_locations (SeahorseSecureEntry *entry,
...
static void
seahorse_secure_entry_adjust_scroll (SeahorseSecureEntry *entry)
{
...
static gint
seahorse_secure_entry_move_visually (SeahorseSecureEntry * entry,
                                     gint start, gint count)
{
    gint _index;
    PangoLayout *layout = seahorse_secure_entry_ensure_layout (entry, FALSE);
    const gchar *text;
...
static gint
seahorse_secure_entry_move_logically (SeahorseSecureEntry *entry,
                                      gint start, gint count)
{
    gint new_pos = start;
...
GtkWidget *
seahorse_secure_entry_new (void)
{
    return g_object_new (SEAHORSE_TYPE_SECURE_ENTRY, NULL);
}

/**
 * seahorse_secure_entry_new_with_max_length:
 * @max: the maximum length of the entry, or 0 for no maximum.
 *   (other than the maximum length of entries.) The value passed in will
 *   be clamped to the range 0-65536.
 *
 * Creates a new #SeahorseSecureEntry widget with the given maximum length.
 *
 ...
 * to seahorse_secure_entry_set_max_length() instead. It is not
 * expected that this function will be removed, but
 * it would be better practice not to use it.
 *
 * Return value: a new #SeahorseSecureEntry.
 **/
GtkWidget *
seahorse_secure_entry_new_with_max_length (gint max)
{
    SeahorseSecureEntry *entry;

    max = CLAMP (max, 0, MAX_SIZE);

    entry = g_object_new (SEAHORSE_TYPE_SECURE_ENTRY, NULL);
...
void
seahorse_secure_entry_set_text (SeahorseSecureEntry *entry, const gchar *text)
{
    gint tmp_pos;
...
void
seahorse_secure_entry_append_text (SeahorseSecureEntry *entry, const gchar *text)
{
    gint tmp_pos;
...
void
seahorse_secure_entry_prepend_text (SeahorseSecureEntry *entry, const gchar *text)
{
    gint tmp_pos;
...
void
seahorse_secure_entry_set_position(SeahorseSecureEntry *entry, gint position)
{
    g_return_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry));
 ...
 **/
void
seahorse_secure_entry_set_invisible_char (SeahorseSecureEntry *entry, gunichar ch)
{
    g_return_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry));
 ...
 * for entries with visisbility set to false. See seahorse_secure_entry_set_invisible_char().
 *
 * Return value: the current invisible char, or 0, if the entry does not
 *               show invisible text at all.
 **/
gunichar
seahorse_secure_entry_get_invisible_char (SeahorseSecureEntry * entry)
{
    g_return_val_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry), 0);
...
seahorse_secure_entry_get_text (SeahorseSecureEntry *entry)
{
    g_return_val_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry), NULL);

    return entry->text;
}

void
seahorse_secure_entry_select_region (SeahorseSecureEntry *entry,
                                     gint start, gint end)
{
 ...
 **/
void
seahorse_secure_entry_set_max_length(SeahorseSecureEntry *entry, gint max)
{
    g_return_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry));
 ...
 * @entry. See seahorse_secure_entry_set_max_length().
 *
 * Return value: the maximum allowed number of characters
 *               in #SeahorseSecureEntry, or 0 if there is no maximum.
 **/
gint
seahorse_secure_entry_get_max_length (SeahorseSecureEntry *entry)
{
    g_return_val_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry), 0);
 ...
 **/
void
seahorse_secure_entry_set_activates_default (SeahorseSecureEntry *entry,
                                             gboolean setting)
{
    g_return_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry));
 ...
 **/
gboolean
seahorse_secure_entry_get_activates_default (SeahorseSecureEntry *entry)
{
    g_return_val_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry), FALSE);
 ...
 **/
void
seahorse_secure_entry_set_width_chars (SeahorseSecureEntry *entry, gint n_chars)
{
    g_return_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry));
 ...
 **/
gint
seahorse_secure_entry_get_width_chars (SeahorseSecureEntry *entry)
{
    g_return_val_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry), 0);
 ...
 **/
void
seahorse_secure_entry_set_has_frame (SeahorseSecureEntry *entry, gboolean setting)
{
    g_return_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry));
 ...
 **/
gboolean
seahorse_secure_entry_get_has_frame (SeahorseSecureEntry *entry)
{
    g_return_val_if_fail (SEAHORSE_IS_SECURE_ENTRY (entry), FALSE);
 ...
 * pixel positions, in combination with seahorse_secure_entry_get_layout_offsets().
 * The returned layout is owned by the entry so need not be
 * freed by the caller.
 *
 * Keep in mind that the layout text may contain a preedit string, so
 * seahorse_secure_entry_layout_index_to_text_index() and
 * seahorse_secure_entry_text_index_to_layout_index() are needed to convert byte
 * indices in the layout to byte indices in the entry contents.
 *
 * Return value: the #PangoLayout for this entry
 **/
PangoLayout *
seahorse_secure_entry_get_layout (SeahorseSecureEntry *entry)
{
    PangoLayout *layout;
...
/**
 * seahorse_secure_entry_layout_index_to_text_index:
 * @entry: a #SeahorseSecureEntry
 * @layout_index: byte index into the entry layout text
 *
 * Converts from a position in the entry contents (returned
 * by seahorse_secure_entry_get_text()) to a position in the
 * entry's #PangoLayout (returned by seahorse_secure_entry_get_layout(),
 * with text retrieved via pango_layout_get_text()).
 *
 * Return value: byte index into the entry contents
 **/
gint
seahorse_secure_entry_layout_index_to_text_index (SeahorseSecureEntry * entry,
                                                  gint layout_index)
{
    PangoLayout *layout;
    const gchar *text;
...
/**
 * seahorse_secure_entry_text_index_to_layout_index:
 * @entry: a #SeahorseSecureEntry
 * @text_index: byte index into the entry contents
 *
 * Converts from a position in the entry's #PangoLayout(returned by
 * seahorse_secure_entry_get_layout()) to a position in the entry contents
 * (returned by seahorse_secure_entry_get_text()).
 *
 * Return value: byte index into the entry layout text
 **/
gint
seahorse_secure_entry_text_index_to_layout_index (SeahorseSecureEntry *entry,
                                                  gint text_index)
{
    PangoLayout *layout;
    const gchar *text;
 ...
 **/
void
seahorse_secure_entry_get_layout_offsets (SeahorseSecureEntry *entry,
                                          gint *x, gint *y)
{
 ...
 */
static void
activate_cb (GtkWidget *menuitem, SeahorseSecureEntry *entry)
{
...
{
    SeahorseSecureEntry *entry = SEAHORSE_SECURE_ENTRY(data);
    seahorse_secure_entry_enter_text (entry, text);
}
#endif
...
static gboolean
cursor_blinks (SeahorseSecureEntry *entry)
{
...
static gint
get_cursor_time (SeahorseSecureEntry *entry)
{
    GtkSettings *settings = gtk_widget_get_settings (GTK_WIDGET (entry));
    gint time;
...
static void
show_cursor (SeahorseSecureEntry *entry)
{
...
static void
hide_cursor(SeahorseSecureEntry * entry)
{
    if (entry->cursor_visible) {
...
blink_cb (gpointer data)
{
    ...
    if (!GTK_WIDGET_HAS_FOCUS (entry)) {
        g_warning ("SeahorseSecureEntry - did not receive focus-out-event. If you\n"
...
static void
seahorse_secure_entry_check_cursor_blink (SeahorseSecureEntry *entry)
{
...
static void
seahorse_secure_entry_pend_cursor_blink (SeahorseSecureEntry *entry)
{
    if (cursor_blinks (entry)) {
